...
import {
  AnaliseSolicitacaoAlteracaoSenhaService
} from "../analise-solicitacao-alteracao-senha-service/analise-solicitacao-alteracao-senha.service";
import {MAT_DIALOG_DATA} from "@angular/material/dialog";

@Component({
  selector: 'app-modal-negar-analise-solicitacao-alteracao-senha',
  templateUrl: './modal-negar-analise-solicitacao-alteracao-senha.component.html',
  styleUrls: ['./modal-negar-analise-solicitacao-alteracao-senha.component.css']
})
export class ModalNegarAnaliseSolicitacaoAlteracaoSenhaComponent implements OnInit {

  motivosReprovacao: any[] = [];
  id: number = 0;

  formNegarAlteracaoSenha: FormGroup = this.formBuilder.group({
  ...
  salvar() {
    ...
    this.submeterNegarAlteracaoSenha(request);
  }

  submeterNegarAlteracaoSenha(request: any) {
    this.utils.loading.next(true);
    this.service.negarSolicitacao(request).subscribe( {
      next: (data: any) => {
        this.utils.loading.next(false);
        this.utils.openAlertModal(
          "320px", "success-modal", "Alteração/Solicitação de senha negada!",
      ...
      error: (data: any) => {
        this.utils.loading.next(false);
        this.utils.openAlertModal(
          "320px", "error-modal", "Erro ao salvar negar alteração de senha",
          `Houve um erro ao tentar negar a alteração de senha em nossa base de dados! Por favor, tente novamente! Caso o problema persista, entre em contato via e-mail: sistemas.supinf@saude.rj.gov.br`
        );
      }
    })
  }
